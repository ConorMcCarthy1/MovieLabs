...
export const getMovie = (args) => {
  ...
  export const getTopCredits = () => { 
   return fetch('https://api.themoviedb.org/3/trending/person/day?api_key='+ process.env.REACT_APP_TMDB_KEY + '&language=en-US')
    .then((response) => {
      if (!response.ok) {
        throw new Error(response.json().message);
      } 
  ...
  export const getCredits = () => {
   return fetch('https://api.themoviedb.org/3/person/person_id?api_key=' + process.env.REACT_APP_TMDB_KEY +  'language=en-US')
  .then(response => response.json())
  .then(response => console.log(response))
  .catch(err => console.error(err));
  }

  export const getCreditImages = ({ queryKey }) => {
    const [, idPart] = queryKey;
    const { id } = idPart;
    return fetch(
      `https://api.themoviedb.org/3/people/${id}/images?api_key=${process.env.REACT_APP_TMDB_KEY}&language=en-US`
    ).then( (response) => {
      if (!response.ok) {
        throw new Error(response.json().message);
      }
